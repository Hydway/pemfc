...
class TransformerModel(nn.Module):
    def __init__(self, input_dim, d_model=256, drop=0, win_size=100):
        ...
        self.block1 = TransformerBlock(d_model=d_model, head_num=4, drop=drop, win_size=win_size)
        self.block2 = TransformerBlock(d_model=d_model, head_num=4, drop=drop, win_size=win_size)
    ...
    def forward(self, x):
        ...
        x = x[:, -1, :]
        x = self.avg_pool(self.fc_out1(x))
        # x = self.drop(x)
        x = self.fc_out2(x)
        # x = self.drop(x)
        return x